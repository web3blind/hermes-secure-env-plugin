...
- Identify the active profile home and the requesting normalized platform plus exact user ID from trusted Hermes session context, never the request text. Treat paths as data, quote every shell argument, and reject control characters. Do not infer ownership from chat access or guess another profile.
...
Run `python -m secure_env_ingress.setup_config --help` from the matching reviewed source in the runtime user's Hermes environment, not as root. Use `configure --home /absolute/profile/home --platform PLATFORM --owner EXACT_USER_ID --public-ip VERIFIED_IP` with trusted normalized identity. `--platform` defaults to `telegram` for legacy numeric-owner compatibility; use the exact platform for every other identity, never map it to Telegram. Optional `--port` selects the listener; `--tls-dir` is the TLS root plus `/current`. Use `check` for value-free read-back and TLS verification. The default `test` target never changes the main `.env` or grants a chat-supplied path. Preserve existing profiles, owner lists and unrelated settings; ask before changing conflicting security policy.
...
- End installation assistance. The owner can use `/senv test` only in a private or operator-trusted chat to exercise the form, and configure additional key names/targets without sharing values. The originating chat receives a bearer link; while an agent is busy, wait or `/stop` before sending a fresh command. Do not ask them to type a real API key into chat.
